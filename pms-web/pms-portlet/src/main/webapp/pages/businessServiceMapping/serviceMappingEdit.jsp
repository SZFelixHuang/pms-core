...
<form action="/pms-portlet/actions/businessServiceMapping/doUpdate" method="post">
	<c:choose>
		<c:when test="${serviceMapping.brandBasic != null}">
			<input type="hidden" id="brandId" name="serviceMapping.brandBasic.id" value="${serviceMapping.brandBasic.id}"/>
			<input type="hidden" name="serviceMapping.brandBasic.level" id="level" value="${serviceMapping.brandBasic.level}" />
		</c:when>
		<c:otherwise>
			<input type="hidden" id="brandId" name="serviceMapping.brandDetail.id" value="${serviceMapping.brandDetail.id}"/>
			<input type="hidden" name="serviceMapping.brandBasic.level" id="level" value="5" />
		</c:otherwise>
	</c:choose>
	<table style="width: auto; min-width: 100%;" id="serviceList">
		<caption style="text-align: left">服务项目映射：</caption>
		<thead>
			  <tr>
			    <th width="30%">服务名称</th>
			    <th width="*">描述</th>
		...
		<tbody>
			<tr>
				<td>
					<input type="hidden" name="serviceMapping.service.id" value="${serviceMapping.service.id}" />
					${serviceMapping.service.serviceName}
				</td>
				<td>${serviceMapping.service.description}</td>
				<td>
					 <c:choose>
		                <c:when test="${serviceMapping.service.enable}">
	...
	<table style="width: auto; width:100%;margin-top: 20px;" id="materialList">
		<caption style="text-align: left">维护材料映射： <a href="javascript:doMaterialLookUp();" style="margin-left: 10px;"><span class="icon-search"></span></a></caption>
		<thead>
			  <tr>
			    <th>品牌</th>
			    <th>品牌名称</th>
		...
		<tbody>
			<c:forEach var="materialMapping" items="${serviceMapping.materialMappings}" varStatus="status">
				<tr>
					<td>
						<input type="hidden" name="serviceMapping.materialMappings[${status.index}].material.id" value="${materialMapping.material.id}"/>
						<c:if test="${not empty materialMapping.material.materialIcon}">
							<img width="80px" height="60px" alt="材料商标" style="margin:10px;" src="/pms-portlet/actions/documentation/getBinaryContent/${materialMapping.material.materialIcon}_x86"/>
						</c:if>
					</td>
					<td>${materialMapping.material.materialBrand}</td>
					<td>${materialMapping.material.materialType}</td>
					<td>${materialMapping.material.materialName}</td>
					<td>${materialMapping.material.displayName}</td>
					<td>
						<input type='text' style='min-width:50px;width:50px;background: #eee;color:#333;text-align:center;' name='serviceMapping.materialMappings[${status.index}].materialAmount' value='${materialMapping.materialAmount}'/>
						<span style="margin-left: 10px;">${materialMapping.material.materialUnit}</span>
					</td>
					<td><span class="glyphicon glyphicon-remove hover-icon" style="cursor:pointer;" title="提示：单击鼠标左键，将会提示是否删除【${material.materialName}】记录" onclick="doDeleteMaterialMapping(this,'${material.materialName}')"></span></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
...
<script type="text/javascript">
	function doUpdate()
	{
		...
		if($serviceId.length == 1)
		{
			...
			{
				pms.confirm(pms.NORMAL, "确认后，服务项目映射为[" + $serviceId.parent().text() + "]将会被更新。确定要提交吗?", function() {
					$("form").submit();
				});
			}
			else
	...
	{
		pms.confirm(pms.DANGER, "确定删除维护材料["+materialName+"]映射吗?", function() {
