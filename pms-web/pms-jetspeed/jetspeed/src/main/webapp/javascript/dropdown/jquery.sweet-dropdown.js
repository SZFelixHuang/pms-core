...
//    $(window).off('resize.dropdown').on('resize.dropdown', $.sweetDropdown.hideAll);
  ...
  $.sweetDropdown.hideAll = function(e, hideException) {
    ...
    if (hideException) {
      ...
      if (!$('[data-dropdown="#' + hideExceptionID + '"]').hasClass('dropdown-open')) {
        el = '.dropdown-menu2:not(#' + hideExceptionID + ')';
        trigger = '[data-dropdown!="#' + hideExceptionID + '"]';
      }
    }
    var $trigger = $('body').find(el).removeClass('dropdown-opened').end().find(trigger);
    $trigger.removeClass('dropdown-open');
    animTimeout = window.setTimeout(function() {
      var returnVal =  $('body').find(el).hide().end();
      if($trigger.length == 1 && $trigger[0].lockedKey)
      {
    	  scrollBar.autoResizePopUpContainer(document, $trigger.attr("data-dropdown").replace("#", ""), $trigger[0].lockedKey);
      }
      return returnVal;
    }, 200);
    return true;
  ...
  showDropdown = function(e) { 
    ...
    window.setTimeout(function() {
    	$trigger[0].lockedKey = scrollBar.autoResizePopUpContainer(document, $trigger.attr("data-dropdown").replace("#", ""));
