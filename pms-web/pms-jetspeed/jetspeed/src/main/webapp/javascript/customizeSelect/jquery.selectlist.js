    ...
    SelectList.prototype={
        ...
        renderSelect: function(element){
            var that = this,
                ...
                selectHTML,
                selectListHTML = '';
            if(that.settings.enableInput)
            {
            	selectHTML = '<div id="' + selectID + '" class="select-wrapper ' + selectClassName + '"><input type="hidden" name="' + selectName + '" value="' + selectedValue + '" /><i class="icon select-down"></i><input type="text" class="select-button" value="' + selectedOptionText + '" /><div class="select-list"><ul>';
            }
            else
            {
            ...
            $(selectID).children('.select-list').css({
                width:  function(){
                    if(!that.settings.width){
                        ...
                        return that.settings.width - 1 + 'px';
                    }
                },
