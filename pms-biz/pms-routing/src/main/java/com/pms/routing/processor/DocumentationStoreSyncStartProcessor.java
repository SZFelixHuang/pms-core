...
public class DocumentationStoreSyncStartProcessor implements Processor
{
	@Autowired
	...
	public void process(Exchange exchange) throws Exception
	{
		...
		newDocumentation.setFileSize(ftpClient.getFileSize(result.get("agency"), result.get("fileKey")));
		newDocumentation.setCreateDate(Calendar.getInstance().getTime());

		DocumentationUtil.deleteSubmittedFile(ftpClient, result.get("fileKey"));
		InputStream in = ftpClient.getFileInputstream(newDocumentation.getAgency(), newDocumentation.getFileKey());
		if (in == null)
		{
			throw new RoutingException("Can not read documentation[" + newDocumentation.getFileKey() + "] from FTP.");
		}
